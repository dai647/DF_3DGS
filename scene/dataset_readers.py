...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, semantic_feature_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        semantic_feature_path = os.path.join(semantic_feature_folder, image_name+'_f.pth') 
        if os.path.exists(semantic_feature_path):
            ...
            # semantic_feature = torch.load(semantic_feature_path).cpu().detach() #for dim512

        else:
            semantic_feature_name = 'not exist'
...
def readColmapSceneInfo(path, foundation_model, images, eval, llffhold=8):
    ...
    sence_name=path.split('/')[-1]
    # semantic_feature_dir=f'autoencoder/Sences/{sence_name}/encode_feature' #for dim 512
    semantic_feature_dir=f'./autoencoder/Sences/{sence_name}/encode_feature_9'


    print(f'--------semantic_feature_dir={semantic_feature_dir}')
 
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, 
                                           images_folder=os.path.join(path, reading_dir), semantic_feature_folder=semantic_feature_dir)
    cam_infos = sorted(cam_infos_unsorted.copy(), key = lambda x : x.image_name)


    if cam_infos[0].semantic_feature !=None:
